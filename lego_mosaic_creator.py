...
# --- Configuration ---
st.set_page_config(page_title="LEGO Mosaic Creator", page_icon="🧱", layout="wide")
...
def main():
    ...
    uploaded_file = st.file_uploader("Upload an image (square works best)", type=["jpg", "jpeg", "png"])
    if uploaded_file:
        st.session_state.uploaded_file_bytes = uploaded_file.read()
        st.session_state.uploaded_file_name = uploaded_file.name

    if 'uploaded_file_bytes' in st.session_state:
        image = Image.open(io.BytesIO(st.session_state.uploaded_file_bytes))
        with col2:
        ...
        with col1:
            ...
            shape_index = st.selectbox("Select Piece Shape:", range(len(shape_options)), format_func=lambda i: shape_options[i])
            if shape_index == 0:
                st.session_state.selected_lego_colors = LEGO_COLORS_ROUND
            elif shape_index == 1:
                st.session_state.selected_lego_colors = LEGO_COLORS_SQUARE
            else:
                st.session_state.selected_lego_colors = LEGO_COLORS_ALL

            size_index = st.selectbox("Select Mosaic Size:", range(len(BASEPLATE_SIZES)), format_func=lambda i: BASEPLATE_SIZES[i]['name'])
            mosaic_size = BASEPLATE_SIZES[size_index]['size']

            if st.button("Generate LEGO Mosaic"):
                with st.spinner("Creating mosaic..."):
                    mosaic_data, color_counts = create_mosaic(image, mosaic_size, st.session_state.selected_lego_colors)
    ...
    if st.session_state.get('mosaic_created') and 'mosaic_data' in st.session_state:
        mosaic_data = st.session_state.mosaic_data
        color_counts = st.session_state.color_counts
        lego_colors_used = st.session_state.selected_lego_colors
        ...
        if mosaic_img:
            st.image(mosaic_img)
            st.markdown(get_image_download_link(mosaic_img, "lego_mosaic.png", "Download Mosaic Image"), unsafe_allow_html=True)
